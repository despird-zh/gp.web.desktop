...
class Chip extends Component {
  ...
  // gary add: support add
  handleTouchTapAddIcon = (event) => {
    // Stop the event from bubbling up to the `Chip`
    event.stopPropagation();
    this.props.onRequestAdd(event);
  };
  ...
  render() {
    ...
    const {
      ...
      onRequestAdd, // eslint-disable-line no-unused-vars,prefer-const
      ...other
    } = this.props;

    const deletable = this.props.onRequestDelete;
    let avatar = null;
    const iconStyle = prepareStyles(Object.assign(styles.deleteIcon, deleteIconStyle));
    ...
    // gary add: support add button
    const addable = this.props.onRequestAdd;
    const addIcon = addable ? (
      <AddIcon
        color={styles.deleteIcon.color}
        style={iconStyle}
